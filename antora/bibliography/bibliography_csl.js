...
function applyBibliography(mapInput, bibliographyFiles, styleID = "ieee", language = "en") {
    ...
    function createBibliography(antoraBibliography, citeproc) {
        ...
        result[0].entry_ids.forEach((id, index) => {
            ...
            const text = entries[index].getElementsByClassName("csl-right-inline")[0] ? entries[index].getElementsByClassName("csl-right-inline")[0].innerHTML.replaceAll(/<i>(.+)<\/i>/g,"__$1__").replaceAll(/<b>(.+)<\/b>/g,"++$1++").replaceAll(/____$/g,"") : null
            if (identifier && text) {
